...
import '../../../core/widgets/progress_ticket_bar.dart';
import '../../../data/models/ticket_model.dart';
import '../../providers/auth_provider.dart';

/// Halaman tracking — satu screen untuk semua role, role-aware di dalam.
/// Ini TETAP halaman push biasa (bukan tab), jadi tombol back tetap ada.
class TrackingScreen extends ConsumerWidget {
  final Ticket ticket;
  const TrackingScreen({super.key, required this.ticket});

  List<Map<String, dynamic>> _getSteps(String role) {
    final processingSub = role == 'helpdesk'
        ? 'Ditugaskan ke kamu'
    ...
    final allSteps = [
      ...
      {'label': 'Sedang Diproses', 'sub': processingSub, 'status': 'active'},
  ...
  int _getProgressPercent(String role) {
    final steps = _getSteps(role);
 ...
 void _onBottomButtonTap(BuildContext context) {
          ...
          children: [
                      ...
                      Container(
                        ...
                        child: Text(_statusLabel(ticket.status),
                            style: GoogleFonts.inter(
                                fontSize: 11, fontWeight: FontWeight.w600, color: _statusColor(ticket.status))),
                      ),
                    ],
                  ),
            ...
            Container(
              ...
              child: ProgressTicketBar(
                percent: _getProgressPercent(role),
                color: _getProgressColor(),
            ...
            OutlinedButton.icon(
              onPressed: () => _onBottomButtonTap(context),
    ...
    switch (stepStatus) {
      ...
      case 'active':
        ...
        circleChild = Container(
            width: 8, height: 8, decoration: const BoxDecoration(color: Colors.white, shape: BoxShape.circle));
        isDashed = true;
        break;
      case 'rejected':
          ...
          children: [
            stepStatus == 'active'
                ...
                : Container(
                    ...
                    decoration: BoxDecoration(shape: BoxShape.circle, color: circleColor),
                    child: Center(child: circleChild)),
            if (!isLast) CustomPaint(size: const Size(2, 50), painter: _LinePainter(color: lineColor, dashed: isDashed)),
          ],
        ),
