...
class _RiwayatScreenState extends State<RiwayatScreen> {
  ...
  List<Ticket> get _filtered {
    if (_filter == 'Semua') {
      return DummyData.tickets.where((t) => t.status == 'Closed' || t.status == 'Rejected').toList();
    }
    return DummyData.tickets.where((t) => t.status == _filter).toList();
  }
  ...
  void _onNavTap(int index) {
    ...
    if (index == 2) Navigator.push(context, MaterialPageRoute(builder: (_) => const BuatTiketScreen()));
    if (index == 4) Navigator.push(context, MaterialPageRoute(builder: (_) => const ProfileScreen()));
  }
          ...
          Padding(
            ...
            child: Container(
              ...
              child: Row(
                ...
                children: [
                  _summaryItem('Total Selesai',
                      '${DummyData.tickets.where((t) => t.status == 'Closed').length}'),
                  Container(width: 1, height: 40, color: Colors.white.withOpacity(0.3)),
                  _summaryItem('Ditolak',
                      '${DummyData.tickets.where((t) => t.status == 'Rejected').length}'),
                ],
              ),
            ),
          ),
