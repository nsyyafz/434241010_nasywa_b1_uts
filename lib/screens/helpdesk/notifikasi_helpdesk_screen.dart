...
class NotifikasiHelpdeskScreen extends StatefulWidget {
  const NotifikasiHelpdeskScreen({super.key});

  @override
  State<NotifikasiHelpdeskScreen> createState() =>
      _NotifikasiHelpdeskScreenState();
}

class _NotifikasiHelpdeskScreenState extends State<NotifikasiHelpdeskScreen> {
      ...
      try {
        ...
        if (mounted) {
          await Navigator.push(
            context,
            MaterialPageRoute(
                builder: (_) => DetailTiketHelpdeskScreen(ticket: ticket)),
          );
        }
      } catch (_) {}
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      bottomNavigationBar:
          BottomNavHelpdesk(currentIndex: 2, onTap: _onNavTap),
    );
  }
}